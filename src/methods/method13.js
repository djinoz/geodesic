...
export default function create2VGeodesicDomeMethod13(radius) {
    ...
    for (let i = 0; i < 5; i++) {
        ...
        const v = addVertex(L1_Radius * Math.cos(angle), L1_Height, L1_Radius * Math.sin(angle));
        topRing.push(v);
    }
    // Level 2: 10 vertices.
    ...
    // Let's generate them.
    const secondRing = []; // The "LONG" connectors (Valleys)
    const thirdRing = []; // The "SHORT" connectors (Peaks?)
    ...
    function getIntersection(v1, d1, v2, d2, angleParam) {
    ...
    for (let i = 0; i < 5; i++) {
        ...
        const direction = new THREE.Vector3(Math.cos(angle) * Math.cos(middleShortAngle), -Math.sin(middleShortAngle), Math.sin(angle) * Math.cos(middleShortAngle)).normalize();
    ...
    for (let i = 0; i < 10; i++) {
        ...
        let angleToUse;
        if (isR1Short && isR2Short)
            angleToUse = bottomShortAngle;
        else if (!isR1Short && !isR2Short)
            angleToUse = bottomLongAngle;
        else
            angleToUse = (bottomShortAngle + bottomLongAngle) / 2;
    ...
    for (let i = 0; i < 5; i++)
        edges.push([apexHub, topRing[i], 'SHORT', 1]);
    // 2. TopRing Perimeter (LONG)
    for (let i = 0; i < 5; i++)
        edges.push([topRing[i], topRing[(i + 1) % 5], 'LONG', 2]);
    ...
    for (let i = 0; i < 5; i++)
        edges.push([topRing[i], thirdRing[i], 'SHORT', 4]);
