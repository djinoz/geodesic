...
export default function create2VGeodesicDomeMethod10(radius) {
    ...
    const sharedV0Vertices = []; // Uppermost vertices, one per pentagon
    const sharedRingVertices = []; // Vertices between adjacent pentagons
    ...
    for (let i = 0; i < 5; i++) {
        ...
        sharedV0Vertices.push(addVertex(v0R * Math.cos(baseAngle), v0H, v0R * Math.sin(baseAngle)));
    }
    ...
    for (let i = 0; i < 5; i++) {
        ...
        const vH = centerH;
        sharedRingVertices.push(addVertex(vR * Math.cos(angleClockwise), vH, vR * Math.sin(angleClockwise)));
    }
    ...
    for (let i = 0; i < 5; i++) {
        ...
        perimeter[2] = addVertex((centerR + 0.5) * Math.cos(v2Angle), 0, // Ground level
        (centerR + 0.5) * Math.sin(v2Angle));
        // V3: Unique vertex
        const v3Angle = baseAngle - (2 * Math.PI / 5) * 1.2;
        perimeter[3] = addVertex((centerR + 0.45) * Math.cos(v3Angle), centerH, (centerR + 0.45) * Math.sin(v3Angle));
