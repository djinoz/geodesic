...
export default function create2VGeodesicDomeMethod11(radius) {
    ...
    for (let i = 0; i < 5; i++) {
        ...
        const centerIdx = addVertex(centerDist * Math.cos(baseAngle), centerHeight, centerDist * Math.sin(baseAngle));
        ...
        perimeter[2] = addVertex(radius * Math.cos(v2Angle), 0, // Ground level
        radius * Math.sin(v2Angle));
        // V3: Unique vertex
        const v3Angle = baseAngle - (2 * Math.PI / 5) * 1.2;
        perimeter[3] = addVertex(radius * 0.95 * Math.cos(v3Angle), radius * 0.25, radius * 0.95 * Math.sin(v3Angle));
    ...
    for (let i = 0; i < 5; i++) {
        ...
        addEdge(bottomPentagons[i].perimeter[1], bottomPentagons[counterClockwise].perimeter[4], 'LONG');
    }
    ...
    const constraint2Pass = shortLengths.every(len => Math.abs(len - SHORT_LENGTH) < tolerance);
    const constraint3Pass = longLengths.every(len => Math.abs(len - LONG_LENGTH) < tolerance);
    if (!constraint2Pass) {
