...
function loadStepFromStorage(): void {
...
function saveStepToStorage(): void {
    try {
...
export function loadAnglesFromStorage(): void {
    try {
        const storedAngles = localStorage.getItem(ANGLES_STORAGE_KEY);
        if (storedAngles) {
            const angles = JSON.parse(storedAngles);
            if (angles.middleLong !== undefined) middleLongAngle = angles.middleLong;
            if (angles.middleShort !== undefined) middleShortAngle = angles.middleShort;
            if (angles.bottomLong !== undefined) bottomLongAngle = angles.bottomLong;
            if (angles.bottomShort !== undefined) bottomShortAngle = angles.bottomShort;
        }
    } catch (error) {
...
export function saveAnglesToStorage(): void {
    try {
        const angles = {
...
// Angle setters
export function setMiddleLongAngle(degrees: number): void {
    middleLongAngle = degrees;
}

export function setMiddleShortAngle(degrees: number): void {
    middleShortAngle = degrees;
}

export function setBottomLongAngle(degrees: number): void {
    bottomLongAngle = degrees;
}

export function setBottomShortAngle(degrees: number): void {
...
function calculateSphereIntersectionPoint(
    v1: THREE.Vector3,
    d1: number,
    v2: THREE.Vector3,
    d2: number,
    preference: 'lower' | 'upper',
    angleParam?: number
): THREE.Vector3 {
...
function calculateGroundPlaneIntersection(
    v: THREE.Vector3,
    d: number,
    angle: number
): THREE.Vector3 {
    ...
    if (discriminant < 0) {
        ...
        return new THREE.Vector3(
            groundRadius * Math.cos(angle),
            0,
            groundRadius * Math.sin(angle)
        );
    }
...
export default function create2VGeodesicDomeMethod12(radius: number): GeodesicData {
    ...
    const edges: [number, number, 'SHORT' | 'LONG', number][] = []; // Added step number to track when edge was created
    ...
    const addEdge = (v1: number, v2: number, type: 'SHORT' | 'LONG', step: number) => {
    ...
    if (currentBuildStep === 1) {
        ...
        const strutLengths: { v1: number; v2: number; label: string; actualLength: number; expectedLength: number }[] = [];
        for (let i = 0; i < 5; i++) {
    ...
    for (let i = 0; i < 5; i++) {
        ...
        const direction = new THREE.Vector3(
            Math.cos(angle) * Math.cos(MIDDLE_SHORT_ANGLE_RAD),
            -Math.sin(MIDDLE_SHORT_ANGLE_RAD),
            Math.sin(angle) * Math.cos(MIDDLE_SHORT_ANGLE_RAD)
        ).normalize();
    ...
    const tempRingVertices: { vertexIndex: number; angle: number; isShort: boolean }[] = [];
    for (let i = 0; i < 5; i++) {
    ...
    if (currentBuildStep === 6) {
        ...
        const ringVerticesOrdered: number[] = tempRingVertices.map(v => v.vertexIndex);
        const ringEndpoints: [number, number][] = [];
        for (let i = 0; i < 10; i++) {
...
function buildFacesFromEdges(
    vertices: THREE.Vector3[],
    edges: [number, number, 'SHORT' | 'LONG', number][],
    currentStep: number,
    debugInfo?: {
        ringVertices?: number[];
        ringEndpoints?: [number, number][];
    ...
    let debugLabels: GeodesicData['debugLabels'] = undefined;
    if (debugInfo) {
        debugLabels = {
        ...
        if (debugInfo.ringVertices && debugInfo.ringEndpoints) {
            const strutEndpoints: { vertexIndex: number; label: string }[] = [];
