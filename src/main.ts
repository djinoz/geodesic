...
function create2VGeodesicDomeMethod3(radius: number) {
...
function create2VGeodesicDomeMethod4(radius: number) {
...
function create2VGeodesicDomeMethod5(radius: number) {
    ...
    const middleLevelVertices: number[] = [];
    for (let i = 0; i < 5; i++) {
...
function create2VGeodesicDomeMethod7(radius: number) {
    ...
    for (let i = 0; i < 10; i++) {
        ...
        const maxDiff = Math.max(
            Math.abs(edge1 - avgEdge),
            Math.abs(edge2 - avgEdge),
            Math.abs(edge3 - avgEdge)
        );
    ...
    baseRing.forEach((vIdx, i) => {
        const connections = vertexConnections.get(vIdx)?.size || 0;
        if (connections !== 4) {
...
function create2VGeodesicDomeMethod9(radius: number) {
...
function create2VGeodesicDomeMethod10(radius: number) {
    ...
    for (let i = 0; i < 5; i++) {
        ...
        const v1 = addVertex(
            bottomOuterRadius * Math.cos(v1Angle),
            bottomOuterHeight,
            bottomOuterRadius * Math.sin(v1Angle)
        );
        perimeter[1] = v1;
        ...
        const v3 = addVertex(
            bottomOuterRadius * Math.cos(v3Angle),
            bottomOuterHeight,
            bottomOuterRadius * Math.sin(v3Angle)
        );
        perimeter[3] = v3;
