...
function create2VGeodesicDomeMethod9(radius) {
    ...
    for (let i = 0; i < 5; i++) {
        const angle = (i * 2 * Math.PI) / 5;
        const v = addVertex(apexRingRadius * Math.cos(angle), apexRingHeight, apexRingRadius * Math.sin(angle));
    ...
    for (let i = 0; i < 5; i++) {
        ...
        const center = addVertex(ring1Radius * Math.cos(baseAngle), ring1CenterHeight, ring1Radius * Math.sin(baseAngle));
        const perimeter = [];
        ...
        for (let j = 0; j < 3; j++) {
            ...
            const v = addVertex(outerRadius * Math.cos(vertexAngle), outerHeight, outerRadius * Math.sin(vertexAngle));
            perimeter[2 + j] = v;
        }
    ...
    for (let i = 0; i < 10; i++) {
        ...
        const center = addVertex(ring2Radius * Math.cos(baseAngle), ring2CenterHeight, ring2Radius * Math.sin(baseAngle));
        const perimeter = [];
    ...
    for (let i = 0; i < 14; i++) {
        ...
        const center = addVertex(ring3Radius * Math.cos(baseAngle), ring3CenterHeight, ring3Radius * Math.sin(baseAngle));
        const perimeter = [];
        ...
        for (let j = (perimeter[0] !== undefined && perimeter[1] !== undefined ? 2 : 0); j < 5; j++) {
            ...
            const baseHeight = 0;
            const v = addVertex(baseRadius * Math.cos(vertexAngle), baseHeight, baseRadius * Math.sin(vertexAngle));
            perimeter[j] = v;
        }
        ...
        for (let j = 0; j < 5; j++) {
            if (perimeter[j] === undefined) {
                ...
                const baseRadius = radius * 1.15;
                const baseHeight = 0;
                perimeter[j] = addVertex(baseRadius * Math.cos(vertexAngle), baseHeight, baseRadius * Math.sin(vertexAngle));
            }
        }
        // Edges from center to perimeter
        for (let j = 0; j < 5; j++) {
...
function create2VGeodesicDomeMethod10(radius) {
    ...
    for (let i = 0; i < 5; i++) {
        const angle = (i * 2 * Math.PI) / 5;
        const v = addVertex(topRingRadius * Math.cos(angle), topRingHeight, topRingRadius * Math.sin(angle));
        topPerimeter.push(v);
    }
    // Top pentagon edges
